...
import { finishWorkout, workoutPlan } from '@/context/userAPI';
...
const StartWorkout = () => {
  ...
  const handleFinishWorkout = async (id: number): Promise<any> => {
    try {
      const response = await finishWorkout(id); // this is the API function
      return response;
    } catch (error: any) {
      console.log(error);
          ...
          <>
            ...
            >
              {exercises.map((exercise) => (
                <TouchableOpacity
                  key={exercise.id}
                  className={`flex-row items-center bg-white p-4 rounded-xl my-1.5 shadow-sm ${completedExercises[exercise.id] ? "bg-[#FFF5EE] border-l-4 border-[#4CAF50]" : ""
                    }`}
                ...
                >
                  ...
                  <View className="flex-1 ml-4">
                    <Text
                      className={`text-base font-bold ${completedExercises[exercise.id]
            ...
            <View className="py-5">
              ...
              <TouchableOpacity
                className={`flex-row justify-center items-center p-4 rounded-xl mb-2.5 shadow ${isWorkoutActive ? "bg-[#FF9800]" : "bg-[#4CAF50]"
                  }`}
              ...
              {allExercisesCompleted && (
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    try {
                      pauseWorkout();
                      const workoutId = Array.isArray(id) ? Number(id[0]) : Number(id); // Ensure id is a number
                      if (isNaN(workoutId)) {
                        console.error("Invalid workout ID:", id);
                        return;
                      }
                      await handleFinishWorkout(workoutId);
